...
export type CustomEvents<T> = Record<`on${NotBuiltinEvent}`, (e: CustomEvent & {
    currentTarget: T;
}) => void>;
...
type HTMLEvents<T> = {
    [key in keyof HTMLElementEventMap as `on${key}`]?: ((e: (HTMLElementEventMap[key] & {
        currentTarget: T;
    })) => void);
};
type SVGEvents<T> = {
    [key in keyof SVGElementEventMap as `on${key}`]?: ((e: (SVGElementEventMap[key] & {
        currentTarget: T;
    })) => void);
};
type MathMlEvents<T> = {
    [key in keyof MathMLElementEventMap as `on${key}`]?: ((e: (MathMLElementEventMap[key] & {
        currentTarget: T;
    })) => void);
};
type HTMLAttributes<T, A> = A & CustomEvents<T> & HTMLEvents<T> & DataAttributes & SkruvAdditionalAttributes<T> & HTMLGlobalAttributes & {
    isSkruvDom?: false;
};
type SVGAttributes<T, A> = A & CustomEvents<T> & SVGEvents<T> & DataAttributes & SkruvAdditionalAttributes<T> & SVGGlobalAttributes & {
    isSkruvDom?: false;
};
type MathMLAttributes<T, A> = A & CustomEvents<T> & MathMlEvents<T> & DataAttributes & SkruvAdditionalAttributes<T> & MathMLGlobalAttributes & {
    isSkruvDom?: false;
};
type AtomAttributes<T, A> = A & {
    isSkruvDom?: false;
};
type SitemapAttributes<T, A> = A & {
    isSkruvDom?: false;
};
...
type CustomElements = {
    [id: `${string}-${string}`]: getHTMLVnode<Record<string, string | number | boolean | object | Function>, HTMLElement, typeof id, Vnode | SkruvRawElement | SkruvCommentElement | SkruvHeaderElement | string | number | boolean>;
};
