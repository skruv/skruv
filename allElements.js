const elements = {
  ...
  Atom: {
    ...
    elements: {
      ...
      link: {
        comment: "", permittedContent: "", attributes: [
          { name: 'href' },
          { name: 'rel' },
          { name: 'type' },
          { name: 'hreflang' },
...
interface Skruv${toElementName(c)}${k}Element extends ${k}Vnode<'${c}', ${document.createElementNS(ns, c).constructor.name}, {${elements[k].elements[c].attributes.map(e => `
${e.comment ? '/** ' + e.comment.split(/\n/).map(e => e.trim()).join('\n').replaceAll(/\n\n\n/g, '\n\n').replaceAll(/\n\n\n/g, '\n\n').replaceAll(/\n\n\n/g, '\n\n') + ' */' : ''}
'${e.name}'?: ${types[e.name] || 'UNKNOWN'} | undefined`).join('')}
}, Any${k}Content> {}`).join('\n') + `
type Any${k}Element = ${Object.keys(elements[k].elements).map(c => `Skruv${toElementName(c)}${k}Element`).join(' | ')}
type Any${k}Content = Any${k}Element | string | number | boolean | Any${k}Content[] | AsyncGenerator<Any${k}Content> | Promise<Any${k}Content> | (() => Any${k}Content)
`
}).join('\n') + `
type AnyElement = ${Object.keys(elements).map(k => `Any${k}Element`).join(' | ')}
type AnyContent = ${Object.keys(elements).map(k => `Any${k}Content`).join(' | ')}
`
