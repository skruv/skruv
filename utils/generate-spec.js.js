...
const types = $$('[href="#cereactions"]').map(e => [e.nextElementSibling?.nextElementSibling?.innerText, e.nextElementSibling?.nextElementSibling?.nextElementSibling?.innerText, e.nextElementSibling?.innerText])
  .filter(e => !!e[0] && !!e[1] && e[2] === 'attribute')
  .map(e => [typeMapping(e[0]), e[1]])
  .filter(e => !!e[0])
...
const parsed = $$('[data-dfn-type="element"]').filter(e => !!e.parentNode.id)
  .map(e => ({
    name: e.innerText,
    attributesElem: document.querySelector(`[id="${e.parentNode.id}"] ~ .element`)
  }))
  .filter(e => !!e.attributesElem)
  .map((e, i) => {
    e.attributes = []
    ...
    e.attributes = e.attributes.filter(e => e.querySelector('code')).map(e => e.querySelector('code').innerText)
      .reduce((p, c) => {
...
'${a}'?: ${SVGAttributesMap[e][a]} | undefined`)
.join('') + '\n' : ''}
}`).join('')}
...
'${a}'?: ${MathMLAttributesMap[e][a]} | undefined`)
.join('') + '\n' : ''}
}`).join('')}
